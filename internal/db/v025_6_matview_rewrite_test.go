...
func TestVersionStampedV0256(t *testing.T) {
	...
	if !strings.Contains(src, `var ToolVersion = "0.27.152"`) {
		t.Error("internal/db/version.go must declare ToolVersion = \"0.27.152\". The tool_version columns and SBOM output read this constant.")
	}
}
