...
This creates 141 tables (99 in `aveloxis_data`, 38 in `aveloxis_ops`, 4 in `aveloxis_scan`) with full parity to Augur's schema. All DDL uses `CREATE ... IF NOT EXISTS` and `ON CONFLICT DO NOTHING`, so `migrate` is safe to run repeatedly.
...
- **`aveloxis_data`** (99 tables + 20 materialized views) — all collected data plus analytics views
