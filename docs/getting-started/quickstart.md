...
This creates 146 tables and 20 materialized views across three PostgreSQL schemas (`aveloxis_data`, `aveloxis_ops`, and `aveloxis_scan`), plus the `aveloxis_augur_data` compatibility views for 8Knot. It is safe to run repeatedly -- all DDL uses `CREATE ... IF NOT EXISTS`.
