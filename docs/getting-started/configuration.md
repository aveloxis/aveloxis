...
| `collection.mailing_list_backfill_months` | integer | `6` | v0.25.7. How many months of history to scan when a list has no checkpoint yet. Bounds the first-pass cost — full-archive backfill (back to project founding) would be enormous on high-volume lists, so the default is a recent window; raise it to deepen history at the cost of a longer first pass. |
