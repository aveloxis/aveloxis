...
Plus 22 materialized views: 20 for 8Knot compatibility when `collection.materialized_views` is enabled (the default), and two Aveloxis-owned supply-chain views on every deployment (their own build and refresh cadence, `collection.supply_chain_refresh_hours`).
...
| Matview rebuild | Weekly (Saturday) | Drains all workers, rebuilds the 20 8Knot materialized views, resumes |
| Supply-chain view refresh | Every `collection.supply_chain_refresh_hours` (default 24) | Refreshes the two supply-chain views; seconds, no drain |
