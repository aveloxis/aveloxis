...
However, 8Knot and other downstream tools reference Augur's column names directly. To maintain backward compatibility, the 20 8Knot-facing materialized views alias their output columns to match Augur's naming convention exactly (the two v0.29.60 supply-chain views have no Augur counterpart).
