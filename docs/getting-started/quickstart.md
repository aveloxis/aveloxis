...
This creates 147 tables and 22 materialized views across three PostgreSQL schemas (`aveloxis_data`, `aveloxis_ops`, and `aveloxis_scan`), plus the `aveloxis_augur_data` compatibility views for 8Knot; the materialized views only when `collection.materialized_views` is enabled (the default). It is safe to run repeatedly -- all DDL uses `CREATE ... IF NOT EXISTS`.
