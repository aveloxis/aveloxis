	...
	for {
		...
		for _, t := range targets {
			...
			if err != nil {
				...
				if updErr := s.store.UpdateRepoMetadata(ctx, t.RepoID, info.Description, info.PrimaryLanguage, info.Languages, info.Status == "Archived", info.ForkedFrom(), info.PlatformRepoID); updErr != nil {
