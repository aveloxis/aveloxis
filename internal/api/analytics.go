...
var metricCatalog = []MetricDef{
	...
	metricDef("upstream_dependencies", "Upstream Code Dependencies",
		"Count of direct RUNTIME-scope manifest dependencies with resolvable releases; detail carries median libyear staleness plus total_count/dev_count/dev_median_libyear companions for the non-runtime (dev/test/build/optional/peer) split.",
		"dependencies", "snapshot",
		"https://chaoss.community/kb/metric-upstream-code-dependencies/",
		"Pairs the raw count with median libyear so 'many but fresh' and 'few but rotten' are distinguishable; the headline covers shipped (runtime) dependencies since v0.27.46 so dev-tooling expansion never inflates it."),
