...
func TestForgeIDAdoptEndToEnd(t *testing.T) {
	...
	pair := `{"old_forge_id":"111","new_forge_id":"222"}`
	if code, _ := do("POST", adoptPath, admin, pair); code != http.StatusNotFound {
		t.Fatalf("nothing observed yet: %d, want 404", code)
	}
	if code, _ := do("POST", adoptPath, admin, ""); code != http.StatusBadRequest {
		t.Errorf("no pair named: %d, want 400", code)
	}
	...
	if code, _ := do("POST", adoptPath, plainTok, pair); code != http.StatusForbidden {
		t.Errorf("a non-admin session: %d, want 403", code)
	}
	if code, _ := do("POST", adoptPath, admin, `{"old_forge_id":"111","new_forge_id":"222","note":"`+strings.Repeat("x", 1001)+`"}`); code != http.StatusBadRequest {
		t.Errorf("a note one byte over the limit: %d, want 400", code)
	}
	if code, _ := do("POST", adoptPath, admin, `{"old_forge_id":"111","new_forge_id":"222","note":"`+strings.Repeat("x", 50000)+`"}`); code != http.StatusBadRequest {
		t.Errorf("a body far past the cap: %d, want 400", code)
	}
	...
	if code, _ := do("POST", adoptPath, admin, pair); code != http.StatusConflict {
	...
	// A note at the limit, every byte spelled as a \u escape, is accepted:
	// the body cap allows for JSON's six-byte escapes.
	escaped := strings.Repeat(`\u0078`, 999) + "from the button"[:1]
	code, body = do("POST", adoptPath, admin, `{"old_forge_id":"111","new_forge_id":"222","note":"`+escaped+`"}`)
	...
	if err != nil || len(stats.ForgeIDChanges) != 1 || stats.ForgeIDChanges[0].Note != strings.Repeat("x", 999)+"f" ||
