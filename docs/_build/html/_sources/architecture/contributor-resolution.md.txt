...
1. **The rename lands on `gh_login`, never on `cntrb_login`.** Every path that observes a rename — `Resolve`, `UpsertContributorFull`, the batch upsert's rename recovery, and `RenameContributorGhLogin` — updates the current-display-name mirror and leaves the first-observed login alone. Because no path re-writes an established `cntrb_login`, the partial unique index cannot trip on a rename at all.
