...
| 2 | `aveloxis dedup-repos --dry-run`, then `aveloxis dedup-repos` until 0 pairs | v0.25.32 (index precondition v0.28.18) | case-variant duplicate repositories; the migrate skips the `uq_repos_repo_git_ci` backstop (or, for self-hosted GitLab instances, `uq_repos_repo_git_ci_gitlab_instances`) until they are drained | only when the migrate warns; needs the new binary's migrate to have built the `email_message` indexes first (it refuses otherwise); re-run `aveloxis migrate --skip-views` afterwards |
