...
Expect a short stream of `migration step ok` log lines. The full schema (147 tables, 20 materialized views, 50+ indexes) lands in two passes — DDL then matview creation; the second pass runs only with `collection.materialized_views` enabled (the default), and the per-package test databases skip it.
