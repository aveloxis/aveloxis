...
```json
{
  ...
  "collection": {
    ...
    "scorecard_max_concurrent": 8,
    "github_max_inflight": 40,
    "github_max_inflight_per_key": 4,
    "github_budget_foreground_reserve_pct": 25,
    "gone_repo_recheck_days": 28,
    "gone_repo_recheck_disabled": false,
...
| `github.base_url` | string | `"https://api.github.com"` | GitHub API base URL. Change this for GitHub Enterprise Server installations. It also names the one host the `github.api_keys` are sent to (v0.29.57), and so the web host a GitHub organization URL must be on: an organization on any other host is refused when added (web, API, CLI loaders, admin approval) and never enumerated, and the periodic refresh skips such a pre-existing row with a WARN. When the base is not public GitHub, the OpenSSF Scorecard subprocess — which resolves its own host — is not lent a token in any mode: remote mode is skipped and a retained clone gets a local run. The base must be `https`: a plaintext base is treated as a foreign host (nothing is lent to it, no github.com organization matches it). **What this setting does NOT change (v0.29.57):** web login always uses public GitHub (`web.github_client_id` is a github.com OAuth app), and a repository URL on an Enterprise host is classified as generic git by the GUI and CLI and collected git-only. Serving public GitHub AND one or more Enterprise hosts on one deployment, each with its own API keys, is designed for a later release as a per-instance list mirroring the GitLab one. |
...
| `collection.workers` | integer | `12` | Number of concurrent collection workers when running `aveloxis serve`. `aveloxis serve` sizes its connection pool from the scheduler's connection demand (v0.29.58; each slot can hold five connections at once), capped by the server's budget, or set outright by `database.pool_max_conns` — see [Database connection pool](../guide/scaling.md#database-connection-pool). |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `collection.materialized_views` | boolean | `true` | Whether this deployment HAS the 20 8Knot materialized views (the two supply-chain views are not governed by this — see `supply_chain_refresh_hours`). They are derived data for 8Knot and the analytics queries; a deployment that never reads them can set this to `false` and skip building them on every `aveloxis migrate`. Setting it `false` does not DROP views that already exist — that is an operator action, not a config one — so a deployment that turns it off keeps refreshing the views it already built, by the weekly schedule and by `aveloxis refresh-views` alike. Both ask the database what it has: on a database without the 8Knot set they say so and do nothing. |
| `collection.matview_rebuild_day` | string | `"saturday"` | Day of the week the scheduler refreshes the 20 8Knot materialized views. Values: `"sunday"`–`"saturday"`, or `"disabled"` / `"none"` / `"off"` to never auto-rebuild. Independent of `aveloxis refresh-views` which always refreshes on demand. A refresh keeps each view's definition; a changed definition is applied only by a plain `aveloxis migrate` (see [Upgrading](upgrading.md)). |
...
| `collection.dev_build_deps` | bool | `false` | v0.27.45 (summary/19 P2): expand Python dependency collection to dev/test/build/optional manifest families — requirements-variant files (`requirements-dev.txt`, `test_requirements.txt`, or another `.txt` directly inside a `requirements/` directory — never `requirements.txt` itself), pyproject `[build-system].requires` / `[project.optional-dependencies]` / PEP 735 `[dependency-groups]` / poetry groups, Pipfile `[dev-packages]`, setup.py `tests_require`/`extras_require`, setup.cfg `[options.extras_require]`. OPT-IN: this is the findings-volume driver — Python dev tooling is stale-pin-dense, so vulnerability counts jump when it flips on. Canary on a small database first. The Go test-only relabel and the per-ecosystem scope relabels (v0.27.44) are always on — they relabel existing rows without adding any. |
...
| `collection.scorecard_token_count` | integer | `0` | v0.27.5. How many key-pool tokens to hand scorecard as a comma-separated `GITHUB_TOKEN` (scorecard round-robins the list per request). `0` (the default) = every USABLE pool token — a key the pool would refuse to `Acquire` (quarantined after repeated 401s, resting on a secondary limit, or — since v0.29.55 — refused by GitHub until its rate-limit reset, on core or GraphQL) is not lent either; `N>0` = the N least-borrowed of those. If no token can be lent (no GitHub keys configured, or every key resting or refused), remote mode is not attempted (v0.29.10): scorecard's remote mode without a token waits up to an hour for GitHub's rate limit, longer than the default 15-minute timeout. Instead the collection phase runs local mode at once on the retained analysis clone; without a clone it skips the repo with a WARN (`skip_reason=no_usable_token` on the phase line) and retries it next cycle. Multi-token is what makes remote-primary mode viable: a moderate repo costs ~40 API calls in remote mode, and spreading them across the pool prevents any single token from being drained by scorecard while collection workers share it. Negative values collapse to 0. Since v0.29.6 the tokens are BORROWED through the pool (`LendTokens`) rather than copied out of it, so the pool knows which keys each subprocess holds. |
| `collection.scorecard_max_concurrent` | integer | `8` | v0.29.6. Concurrent scorecard SUBPROCESSES across the whole worker pool. A subprocess cannot hold a pool lease, so it is bounded here instead. Derivation: 8 subprocesses × ~40 remote calls each sits inside one key's per-minute secondary budget. A worker whose repo reaches the scorecard phase while all slots are taken waits (the phase is inline in the job by design — a finished cycle is fully finished). Non-positive falls back to 8. |
| `collection.github_max_inflight` | integer | `40` | v0.29.6. Pool-wide ceiling on requests in flight against the forge, across every subsystem (collection workers, the history sweep, refreshers, gap fill). Derived by Little's Law from the fleet: 54 keys × 5,000 points/hr = 75 points/sec, at the measured ~341 ms per GraphQL query ≈ 26 in flight saturates the ENTIRE budget — more concurrency cannot buy throughput, only secondary-limit rejections. 40 is 1.5× headroom and 2.5× under GitHub's ~100-concurrent secondary limit. Applied to the GitLab pool too as a conservative backstop. Non-positive falls back to 40. |
| `collection.github_max_inflight_per_key` | integer | `4` | v0.29.6. Per-key ceiling on requests in flight. Non-binding at 54 keys under a global 40 (~0.7 per key); it is the structural guarantee that load can never concentrate onto one token however the pool is otherwise depleted (the 2026-09-12 analysis measured up to 96% of 192 concurrent history-sweep requests landing on ONE key). It also bounds per-key points/minute by construction: 4 in flight at ≥ 120 ms latency is ≤ 2,000/min, GitHub's GraphQL secondary line. Non-positive falls back to 4. GitHub only: the GitLab pool carries no per-key ceiling (GitLab has no per-token concurrency limit of this shape, and a one-token GitLab fleet would otherwise be capped at 4 concurrent requests across every worker). |
| `collection.github_budget_foreground_reserve_pct` | integer | `25` | v0.29.6. Share of the pool's hourly budget that BACKGROUND sweeps (contributor history and classification) may not spend into; foreground collection is admitted while any budget remains. Replaces the v0.29.0 per-key 500-point cliff, which shrank the background-eligible key set as keys depleted and funnelled the whole sweep onto the survivors. Derived: collection measured at ~9% of budget, × ~3 safety. Background is admitted while the usable pool total (keys resting on a secondary limit or a 401 quarantine do not count, and since v0.29.55 neither does the tracked balance of a key GitHub has refused) is ABOVE the line. At or below it — while at least one usable key still holds spendable budget — a WAITING sweep (contributor history) waits for the next window reset, the next resting key's return or the next refusal's end and logs `GraphQL background sweep paced by the foreground reserve`; the FAST-FAILING classification sweep instead receives `ErrGraphQLBudgetExhausted` and its ticker retries next interval — nothing is logged by the pool; the scheduler's `activity classification: some chunks failed — fetched rows kept, unanswered logins retry next tick` WARN is where that starvation is visible. When no usable key holds spendable budget the pool is exhausted, not paced: the waiting sweep logs the same `all API keys exhausted for GraphQL` line foreground callers see, and the fast-failing sweep again gets the error rather than a log line. Absent or non-positive falls back to 25; explicit values are clamped to [1, 99] — the reservation is never a disable switch in either direction (1 = nominal; 100 would equal a full pool and switch background off entirely). |
| `collection.gone_repo_recheck_days` | integer | `28` | v0.29.7. Per-repo cadence on which `aveloxis serve` re-verifies a "gone" repository against its forge. A repository whose URL returned a definitive 404/410/451 is dequeued, so no collection cycle ever probes it again — without this ticker a repository made private and later public again stayed gone until an operator ran `aveloxis mark-gone-repos`. Every hour the scheduler claims up to 500 gone-stamped repos whose last check is older than this many days (never-checked rows first), probes each with the same unauthenticated `HEAD` prelim uses (no API-key budget), and on a definitive 2xx clears the gone state and re-enqueues the repo; a 404/410/451, an indeterminate status (403/429/5xx) or a transport error restamps the check and the repo waits another cadence (the probe already makes four attempts on transient network errors, and an unbounded retry-next-tick arm would let a permanently unreachable cohort head every claim). The 500-per-hour batch cycles a 300K-repo gone cohort inside the 28-day default when the hosts answer (a HEAD to github.com takes ~200 ms). A host that never answers costs up to 73 seconds per repository (the probe's retry ladder), once per cadence, so the cadence holds for at most `gone_repo_recheck_days × 86,400 / 73` such repositories (about 33,000 at the default of 28, about 1,200 at 1). Any single cycle that runs longer than its hour logs `gone recheck cycle overran its tick` with the achieved `probes_per_hour`. About 50 unreachable repositories in one batch are enough to trigger it, so the warning means "this batch was slow", not "the cadence slipped". Absent or non-positive falls back to 28 — `0` is not a disable switch; values above 365 are clamped to 365. |
| `collection.gone_repo_recheck_disabled` | boolean | `false` | v0.29.7. Set `true` to switch the recheck ticker off; `aveloxis mark-gone-repos` remains the manual path. |
