...
aveloxis refresh-views          # rebuild materialized views + dm_ aggregates
aveloxis start all
```
...
Migration steps are fail-closed (v0.19.4): a failing step aborts the
migrate, the error lists **every** failed step, and `serve` refuses to
start until they are fixed. Three steps are deliberately warn-only and
wait for operator action instead:
...
| 2 | `aveloxis dedup-repos --dry-run`, then `aveloxis dedup-repos` until 0 pairs | v0.25.32 | case-variant duplicate repositories; the migrate skips the `uq_repos_repo_git_ci` backstop until they are drained | only when the migrate warns; re-run `aveloxis migrate --skip-views` afterwards |
...
| 8 | `aveloxis heal-collection-gaps --dry-run`, then `--workers 4` until 0 candidates | v0.27.140 (safe beside `serve` from v0.27.150) | issues / PRs lost to the pre-v0.27.139 blind-window `since` bug; visits only repos whose metadata counts exceed stored counts | **required** for any repo collected before v0.27.139; must run on the new binary |
| 9 | `aveloxis refresh-views` | — | materialized views and `dm_` aggregates over the healed data | after rows 3–8 settle (the weekly rebuild also covers it) |
| 10 | `aveloxis reconcile-repos` | v0.27.39 | stranded repositories (a `repos` row with no queue row) left by upstream renames | periodic, until the residue drains |
