...
// TestAcquireStampsProbeWhenResetUnknown (Copilot round 22 on
...
func TestAcquireStampsProbeWhenResetUnknown(t *testing.T) {
	...
	if got.IsZero() {
		t.Fatal("Acquire must stamp a bounded probe deadline on an exhausted key with no known reset — else the refill guard never fires and callers poll forever (round 22)")
	}
