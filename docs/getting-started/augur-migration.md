...
- **`aveloxis_data`** -- 102 tables + 22 materialized views (20 for 8Knot when `collection.materialized_views` is enabled, the default; two supply-chain views always) for collected data
...
| `aveloxis_data` | Aveloxis | Aveloxis collected data (102 tables + 22 matviews) |
