...
| `collection.vuln_scan_transitive` | bool | `false` | Phase C1 (v0.27.21): scan the FULL transitive closure each committed lockfile enumerates, in addition to declared direct dependencies. Transitive findings carry `dependency_kind='transitive'` and render behind a toggle in the GUI. OPT-IN: leave off until the canary numbers (findings growth, OSV cache hit rate, scan duration) validate your fleet — see summary/14. Flipping it off later resolves the transitive findings on each repo's next scan (rows remain as historical record). |
